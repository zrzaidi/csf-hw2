...
uint32_t compute_index(struct Image *img, int32_t x, int32_t y) {
  return img->width * x + y;
}

// uint32_t r_blend(struct Image *img, int32_t x, int32_t y, uint32_t color) {
...
void draw_pixel(struct Image *img, int32_t x, int32_t y, uint32_t color) {
  ...
  uint32_t newColor = 0;
  newColor = ((get_a(color)*get_r(color) + (255 - get_a(color)) * get_r(background)) / 255) << 8;
  newColor = (newColor | ((get_a(color)*get_g(color) + (255 - get_a(color)) * get_g(background)) / 255)) << 8;
